...
const socialButtons = [
  ...
  { key: 'whatsapp',  Icon: IconWhatsApp,  label: 'WhatsApp',  color: '#25D366' },
];
...
const VideoCard = ({ cat, index }: { cat: any; index: number }) => {
  ...
  return (
    <motion.div
      ...
      className="group relative overflow-hidden bg-cream p-6 md:p-10 border border-[rgba(160,140,120,0.15)] shadow-sm transition-shadow duration-300 hover:shadow-[0_30px_60px_rgba(20,17,14,0.15)]"
    >
      ...
      {/* Video */}
      <motion.div
        className="relative aspect-[4/3] bg-dark overflow-hidden shadow-inner"
      ...
      {/* Botones redes sociales — aparecen al hover */}
      <div className="flex gap-3 mt-6 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0" style={{ transform: "translateZ(30px)" }}>
        {socialButtons.map(({ key, Icon, label, color }, i) => (
          <motion.a
            key={key}
            ...
            className="w-9 h-9 rounded-full border border-[rgba(160,140,120,0.3)] flex items-center justify-center text-mid bg-cream/80 hover:border-transparent transition-colors duration-200"
            style={{ color: 'inherit' }}
          >
            <Icon />
          </motion.a>
